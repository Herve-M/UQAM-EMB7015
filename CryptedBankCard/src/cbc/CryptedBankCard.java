...
//        cipher.doFinal(buffer,ISO7816.OFFSET_CDATA,byteRead,buffer,ISO7816.OFFSET_CDATA);
    ...
    private void insSetOwnerPin(APDU apdu) {
        byte[] buffer = apdu.getBuffer();
        JCSystem.beginTransaction();
        ownerPIN.update(buffer, ISO7816.OFFSET_CDATA, buffer[ISO7816.OFFSET_LC]);
        JCSystem.commitTransaction();
    }
    ...
    void insSetPublicModulus(APDU apdu){
        ...
        try {
            JCSystem.beginTransaction();
            publicKey.setModulus(buffer, ISO7816.OFFSET_CDATA, buffer[ISO7816.OFFSET_LC]);
            JCSystem.commitTransaction();
    ...
    */
    void insSetPrivateModulus(APDU apdu){
        ...
        try{
            JCSystem.beginTransaction();
            privateKey.setModulus(buffer, ISO7816.OFFSET_CDATA, buffer[ISO7816.OFFSET_LC]);
            JCSystem.commitTransaction();
        }
        catch(CryptoException ex){
            JCSystem.abortTransaction();
    ...
    void insSetPrivateExp(APDU apdu){
        ...
        try{
            JCSystem.beginTransaction();
            privateKey.setExponent(buffer, ISO7816.OFFSET_CDATA, buffer[ISO7816.OFFSET_LC]);
            JCSystem.commitTransaction();
        }
        catch(CryptoException ex){
            JCSystem.abortTransaction();
